# ☁️ AWS Cloud Odyssey

### *Un viaje por arquitecturas AWS de producción*

Bienvenido a **AWS Cloud Odyssey**, una serie técnica en español dedicada al diseño de arquitecturas reales sobre Amazon Web Services.

Aquí no quiero limitarme solamente a explicar qué hace cada servicio. El objetivo es documentar **cómo pienso una arquitectura cuando la llevo a un escenario de producción**: qué requisito intento resolver, qué riesgo estoy aceptando, qué alternativa descartaría, qué impacto puede tener en operación y costos.

## 🧠 Perspectiva desde la experiencia

Cada capítulo separa el patrón de referencia, mi decisión como arquitecto y lo que validaría antes de producción. La intención es que cada arquitectura pueda defenderse en una conversación técnica: no solo mostrar servicios, sino explicar por qué están ahí, qué problema resuelven y qué cambiaría bajo otras condiciones.

## 🧭 La Odyssey

| Capítulo | Misión | Estado |
|---|---|---|
| **01** | 🌐 [El Perímetro Global — Entrada segura a una aplicación AWS](./capitulos/01-el-perimetro-global/README.md) | 🚀 Publicado |
| **02** | 🏗️ [Construyendo para Producción — Arquitectura Multi-AZ](./capitulos/02-construyendo-para-produccion/README.md) | 🚀 Publicado |
| **03** | ☸️ [Kubernetes a Escala — Amazon EKS](./capitulos/03-kubernetes-a-escala/README.md) | 🚀 Publicado |
| **04** | 🛡️ Defensa en Profundidad — Seguridad AWS | Próximamente |
| **05** | ⚡ El Mundo Serverless | Próximamente |
| **06** | 🤖 La Era GenAI — Amazon Bedrock | Próximamente |
| **07** | 📊 El Camino de los Datos | Próximamente |
| **08** | 🔭 Observando la Nube | Próximamente |
| **09** | 💰 Arquitectura con FinOps | Próximamente |
| **10** | 🏢 La Organización Cloud | Próximamente |
| **11** | 🌎 Sobreviviendo a una Región | Próximamente |
| **12** | 🏆 La Arquitectura Final | Próximamente |

## 👤 Sobre el autor

**Juan Gutierrez**  
Solutions Architect enfocado en Cloud Architecture, Kubernetes, Security, FinOps y Cloud Modernization.

🔗 [LinkedIn](https://www.linkedin.com/in/juan-gutierrez25)  
💻 [GitHub](https://github.com/juan2521)

---

**Juan Gutierrez** · AWS Cloud Odyssey · Arquitectura · Seguridad · Resiliencia · Escalabilidad · FinOps · Infrastructure as Code
